...
- 🔲 **Rutas de dominio aún MOCK:** chat / ingestions / settings siguen sobre `server/utils/mock-db.ts` **hasta que el backend exponga esos dominios**. Al existir, se reemplaza cada handler por `backendFetch(event, '/api/...')` (el cliente no cambia).
