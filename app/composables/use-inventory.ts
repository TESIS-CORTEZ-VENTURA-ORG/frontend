...
import type { Ingredient, InventoryMovement, MovementType, ShoppingItem, StockStatus } from '#shared/types/domain'
...
export function useStockLevels() {
  return useQuery({
    key: ['stock-levels'],
    query: () =>
      $fetch<ApiResponse<StockLevel[]>>('/api/inventory/stock').then(r => r.data),
  })
}
...
export function useCreateMovement() {
  ...
  return useMutation({
    ...
    mutation: (payload: { ingredientId: string, type: MovementType, qty: number, note?: string, reason?: string, user?: string }) =>
      $fetch<ApiResponse<{ movement: InventoryMovement, ingredient: Ingredient | undefined }>>(
...
export function useAlerts() {
  return useQuery({
    ...
    query: () =>
      $fetch<ApiResponse<StockAlert[]>>('/api/inventory/alerts').then(r => r.data),
  })
}
...
export function useWaste() {
  return useQuery({
    key: ['inventory-waste'],
    query: () =>
      $fetch<ApiResponse<WasteHistory>>('/api/inventory/waste').then(r => r.data),
  })
}
...
function useShoppingOverlay() {
  return useState<ShoppingOverlay>('shopping-overlay', () => ({ checked: {}, extra: [] }))
}
...
export function useShoppingList() {
  ...
  const data = computed<ShoppingItem[]>(() => {
    ...
    const extra = overlay.value.extra.filter(e => !seedIds.has(e.ingredientId))
...
export function usePatchShoppingItem() {
  const overlay = useShoppingOverlay()
  return useMutation({
    mutation: ({ id, checked, suggestedQty }: { id: string, checked?: boolean, suggestedQty?: number }) => {
      // `id` === ingredientId (la lista no se persiste).
      if (checked !== undefined) overlay.value.checked = { ...overlay.value.checked, [id]: checked }
...
export function useAddShoppingItem() {
  ...
  return useMutation({
    ...
    mutation: async (payload: { ingredientId: string, suggestedQty?: number }) => {
      const ingredients = await $fetch<ApiResponse<Ingredient[]>>('/api/ingredients').then(r => r.data)
      const ing = ingredients.find(i => i.id === payload.ingredientId)
      if (!ing) throw new Error('Insumo no encontrado')
      const qty = payload.suggestedQty
        ?? Math.max(1, +(ing.minStock * 2 - ing.stock).toFixed(2))
      const existing = overlay.value.extra.find(x => x.ingredientId === ing.id)
      if (existing) {
        existing.suggestedQty = +(existing.suggestedQty + qty).toFixed(2)
        existing.estimatedCost = +(existing.suggestedQty * ing.unitCost).toFixed(2)
