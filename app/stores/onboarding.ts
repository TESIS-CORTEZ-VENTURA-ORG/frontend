...
export const useOnboardingStore = defineStore('onboarding', {
  state: (): OnboardingState => ({
    ...
    emailVerified: false,
    restaurant: { name: '', cuisine: '', address: '', phone: '', ruc: '', logoName: null },
    setup: defaultSetup(),
    setupSkipped: false,
    importDone: false,
    registered: false,
  ...
  getters: {
    firstName: (state): string => state.account.name.trim().split(/\s+/)[0] ?? '',
    totalTables: (state): number => state.setup.areas.reduce((sum, a) => sum + a.tables, 0),
  },
