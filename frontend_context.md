...
- 🔲 **UI nueva de features E02 aún pendiente (backend listo):** crear insumo + **importar CSV** (HU-02-02) en Stock; **modificadores** (HU-02-11), **disponibilidad horaria** (HU-02-13) y **alerta de margen <25%** (HU-02-10) en el detalle de receta. Sin pantalla aún: admin de **unidades** (HU-02-03), **proveedores** (HU-02-05/06) y **categorías de insumo** (HU-02-04).
- 🔲 **Rutas de dominio aún MOCK:** inventory / sales / tables / orders / chat / ingestions / settings siguen sobre `server/utils/mock-db.ts` **hasta que el backend exponga esos dominios (E03–E05…)**. Al existir, se reemplaza cada handler por `backendFetch(event, '/api/...')` (el cliente no cambia).
